...
class _EmailSignInFormBlocBasedState extends State<EmailSignInFormBlocBased> {
  ...
  void _toggleFormType() {
    widget.bloc.toggleFormTye();
    _emailController.clear();
    _passwordController.clear();
  }

  void _emailEdittingComplete(EmailSignInModel model) {
  ...
  List<Widget> _buildChildren(EmailSignInModel model) {
    return [
      _buildEmailTextFile(model),
      SizedBox(height: 8.0),
      _buildPasswordTextFile(model),
      SizedBox(height: 8.0),
      FormSubmitButton(
  ...
  TextField _buildEmailTextFile(EmailSignInModel model) {
    ...
    return TextField(
      ...
      onEditingComplete: () => _emailEdittingComplete(model),
      onChanged: (email) => widget.bloc.updateEmail(email),
    );
  }

  TextField _buildPasswordTextFile(EmailSignInModel model) {
    return TextField(
